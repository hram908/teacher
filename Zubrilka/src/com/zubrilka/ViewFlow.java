...
//                R.styleable.ViewFlow);
        mSideBuffer = 10;//styledAttrs.getInt(R.styleable.ViewFlow_sidebuffer, 3);
        init();
    }
    ...
    private void snapToScreen(int whichScreen) {
        ...
        if (!mScroller.isFinished())
            return;
        int duration=Math.abs(whichScreen-mCurrentScreen);
        ...
        if(duration!=0)
            mScroller.startScroll(getScrollX(), 0, delta, 0, Math.abs(delta) *2/duration);
        else
            mScroller.startScroll(getScrollX(), 0, delta, 0, Math.abs(delta) *2);
        invalidate();
        changeBackgroundLesson(whichScreen,mCurrentScreen);
    }
...
//    }
    ...
    private void changeBackgroundLesson(int currentScreen,int lastScreen)
    {
        if(currentScreen!=lastScreen)   {
        TextView current=(TextView)mll_lessons.getChildAt(currentScreen);
        current.setBackgroundResource(R.drawable.lesson_bg_checked);
        current.setTextColor(getResources().getColor(R.color.white));
        TextView previosLesson=(TextView)mll_lessons.getChildAt(lastScreen);
        previosLesson.setBackgroundResource(R.drawable.lesson_bg);
        previosLesson.setTextColor(getResources().getColor(R.color.color_numbers));
        }

    }
    public void snapToSlide(int number_slide)
    {
        if(number_slide!=mCurrentScreen)  {
        snapToScreen(number_slide);
        }
    }
    public void setLinearLayoutLessons(LinearLayout linearLayout)
    {
        mll_lessons=linearLayout;
    }
